...
3.  Only powers of 10 (I, X, C, M) can be appended consecutively at most
...
-   The loop runs through at most **13 values**.
-   Each subtraction reduces `num` significantly (1000, 900, etc.), so
    the number of iterations is bounded by a constant.\
-   **Time Complexity: O(1)**\
-   **Space Complexity: O(1)** (only uses fixed arrays).
